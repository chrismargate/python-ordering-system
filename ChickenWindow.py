...
class ChickenWindow(ProductCategoryWindow):

    list_bag = []

    def __init__(self,master=None):
        super().__init__(master)
        self.folder_name = "Chicken"
        self.food_id = 3000
    ...
    def update_list(self):
        ...
        for instance_item in self.list_bag_instance:
            duplicate = False
            if ChickenWindow.list_bag == []:
                temp_list.append(instance_item)
                continue
            for class_item in ChickenWindow.list_bag:
        ...
        print("in")
        print(temp_list)
        ChickenWindow.list_bag.extend(temp_list)
