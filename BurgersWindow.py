...
class BurgersWindow(ProductCategoryWindow):

    list_bag = []

    def __init__(self,master=None):
        super().__init__(master)
        self.folder_name = "Burgers"
        self.food_id = 2000
    ...
    def update_list(self):
        ...
        for instance_item in self.list_bag_instance:
            duplicate = False
            if BurgersWindow.list_bag == []:
                temp_list.append(instance_item)
                continue
            for class_item in BurgersWindow.list_bag:
        ...
        print("in")
        print(temp_list)
        BurgersWindow.list_bag.extend(temp_list)
