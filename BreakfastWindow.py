...
class BreakfastWindow(ProductCategoryWindow):

    list_bag = []

    def __init__(self,master=None):
        super().__init__(master)
        self.folder_name = "Breakfast"
        self.food_id = 1000
    ...
    def update_list(self):
        ...
        for instance_item in self.list_bag_instance:
            duplicate = False
            if BreakfastWindow.list_bag == []:
                temp_list.append(instance_item)
                continue
            for class_item in BreakfastWindow.list_bag:
        ...
        print("in")
        print(temp_list)
        BreakfastWindow.list_bag.extend(temp_list)
