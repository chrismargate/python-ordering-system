...
class DessertsWindow(ProductCategoryWindow):

    list_bag = []

    def __init__(self,master=None):
        super().__init__(master)
        self.folder_name = "Desserts"
        self.food_id = 6000
    ...
    def update_list(self):
        ...
        for instance_item in self.list_bag_instance:
            duplicate = False
            if DessertsWindow.list_bag == []:
                temp_list.append(instance_item)
                continue
            for class_item in DessertsWindow.list_bag:
        ...
        print("in")
        print(temp_list)
        DessertsWindow.list_bag.extend(temp_list)
